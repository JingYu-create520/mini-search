...
A local-first hybrid search engine for Chinese. Crawler, Chinese analyser, inverted index, BM25, word vectors, HNSW, RRF fusion, web UI — all written by hand, packaged as one 227 KB jar with no runtime dependencies.
...
It is not the default and it is not in the jar: ONNX Runtime is a `provided` dependency, so the base artifact stays 227 KB with zero runtime dependencies. Trading one extra `-cp` for the strongest semantic layer is a decision you should make per deployment, which is why it is a flag and not a default.
