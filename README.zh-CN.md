...
单个 180 KB 的 jar，**运行时零依赖、零外部服务、零模型下载**。数据全在你机器上，而且每一层都小到能被一个人读完。
...
| hybrid（RRF 融合） | **0.934** | 0.216 | **0.918** | **0.923** | 36/38 |

按查询类型拆开看更说明问题——`semantic` 类查询被刻意写成"目标文档里不出现查询词"：
...
还有 2 条查询在 top5 里全空："小孩高烧不退是细菌感染吗"（目标是《感冒和流感的区别》）、"为什么海边城市冬天不太冷"（目标是《海洋如何调节气候》）。这两条是真正的同义词/常识鸿沟，纯词法方法和共现词典都跨不过去，需要预训练嵌入——见下面"向量层的门槛"。
...
（把预训练嵌入接进来是留给 v2 的口子：`vector/Encoder` 是个接口，实现一个 ONNX/bge 的编码器不需要动任何其他层。目前**没有**内置模型，因此本项目的向量层完全依赖语料规模。）
...
- 同义词鸿沟真实存在：README 里点名的那 2 条查询就是。
