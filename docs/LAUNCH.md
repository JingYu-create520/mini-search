...
```
Local-first hybrid search engine for Chinese: hand-written analyser, inverted index (BM25), distributed thesaurus, self-trained word vectors + HNSW, RRF fusion. One 225KB jar, zero runtime dependencies, no model download.
```
...
```
Show HN: A local-first Chinese hybrid search engine in a 225KB jar
```
...
JSON parser. No Lucene, no HanLP, no Spring, no vector DB, no model download. Total: 6,591 lines of
main code in 39 files, and the jar is 225KB.
...
build: the jar is attached to the latest release (225KB, one file).
```
...
fusion -> web UI. One 225KB jar. No Lucene, no HanLP, no Spring, no vector DB, and no model download,
...
```
我从零写了一个能搜中文的搜索引擎：6591 行、零依赖、225KB 一个 jar
```
...
```
[开源] 本地优先的中文混合搜索引擎：自研分词+倒排+BM25+HNSW+RRF，6591 行零依赖，一个 225KB jar
```
...
也没有任何需要下载的模型（想要最强语义可以一条命令加装 24MB 的本地 bge 模型，仍然离线）。主代码 6591 行 / 39 个文件，jar 225KB，运行时零依赖。
...
- ✅ 69 个测试全绿；`mvn -B package` 出 225KB jar；无关目录下 `java -jar` 冷启动可搜
