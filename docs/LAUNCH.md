...
JSON parser. No Lucene, no HanLP, no Spring, no vector DB, no model download. Total: 6,601 lines of
...
```
我从零写了一个能搜中文的搜索引擎：6601 行、零依赖、225KB 一个 jar
```
...
```
[开源] 本地优先的中文混合搜索引擎：自研分词+倒排+BM25+HNSW+RRF，6601 行零依赖，一个 225KB jar
```
...
也没有任何需要下载的模型（想要最强语义可以一条命令加装 24MB 的本地 bge 模型，仍然离线）。主代码 6601 行 / 39 个文件，jar 225KB，运行时零依赖。
...
- ✅ 70 个测试全绿；`mvn -B package` 出 225KB jar；无关目录下 `java -jar` 冷启动可搜
