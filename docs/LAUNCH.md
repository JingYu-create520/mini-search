...
```
Local-first hybrid search engine for Chinese: hand-written analyser, inverted index (BM25), distributed thesaurus, self-trained word vectors + HNSW, RRF fusion. One 205KB jar, zero runtime dependencies, no model download.
```
...
```
Show HN: A local-first Chinese hybrid search engine in a 205KB jar
```
...
main code in 36 files, and the jar is 205KB.
...
fusion -> web UI. One 205KB jar. No Lucene, no HanLP, no Spring, no vector DB, and no model download,
...
```
我从零写了一个能搜中文的搜索引擎：5573 行、零依赖、205KB 一个 jar
```
...
```
[开源] 本地优先的中文混合搜索引擎：自研分词+倒排+BM25+HNSW+RRF，5573 行零依赖，一个 205KB jar
```
...
也没有任何需要下载的模型。主代码 5573 行 / 36 个文件，jar 205KB，运行时零依赖。
...
- ✅ 49 个测试全绿；`mvn -B package` 出 205KB jar；无关目录下 `java -jar` 冷启动可搜
