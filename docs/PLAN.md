...
- 上面那行是 2026-09-21 的快照，之后又长过两次，而 README/文章里的表没人回头改——于是同一份文档里同时出现过 6005 和 6104。加了 `scripts/check-claims.sh`：把两份 README、掘金文章和 `docs/LAUNCH.md` 里所有自指数字（分模块行数、主代码行数、文件数、测试行数、用例数、前端行数、最长单文件行数）跟源码树对一遍，对不上就 `exit 1`，CI 每次构建都跑。当前值：6104 行 / 38 文件 / 57 用例。
- 顺带改掉一处文章里的过头话："每层都不到 800 行"在 `vector/` 1086 行面前不成立，换成"单个文件最长 453 行"，这条也在校验脚本里。（后来这批改动把 `core/Engine` 顶到了 577 行，脚本自动把那句话跟着改了——写死数字的地方一旦不再等于树里的数，CI 就红。）

同一天下午又找出四个真问题，都已修：

| # | 问题 | 症状 | 处理 |
|---|---|---|---|
| 1 | 规模表的阶段耗时只打在 stderr，没人能核对 | 文章里"HNSW 建图 156 秒"其实是向量层的**累计**时间；旧 artifact 总构建 201 秒和表里 13.6 秒也来自两次不同运行 | `Engine` 记录各阶段秒数，`bench` 写进 `bench.json` 的 `stages`；README 两张规模表逐格与 artifact 对齐（`check-claims.sh`），并把"十毫秒级"改成"几十毫秒级 ±20%" |
| 2 | 读者与写者共用一把不存在的锁 | `Engine` 的注释写着"读者不需要锁，只取 volatile 的 searcher"，但 searcher 指向的倒排索引是原地改的：`POST /api/index` 与一次查询可以同时在跑 | 一把 `ReentrantReadWriteLock`：写侧包住增删/重建/`stats`/`save`，读侧让一次查询整体持读锁；`/api/doc` 改走带锁访问器。新增 `ConcurrencyTest`（并如实写明：去掉读锁它也不会红——x86 上这个竞态不复现，锁是为了语义不依赖编译器） |
| 3 | 爬虫是个服务端请求出口 | 绑定 `0.0.0.0` 或跑 Docker 时，任何人 `POST /api/crawl` 就能让服务端去抓 `127.0.0.1`、`169.254.169.254`、`10.x`，正文进了索引再通过 `/api/search` 读回去 | 新增 `crawl/UrlPolicy`：默认拒绝非 http(s)、解析不出的域名，以及环回/`0.0.0.0/8`/链路本地/RFC1918/CGNAT/IPv6 ULA/组播；重定向落地再查一次。`--allow-private`、`--allow-private-crawls` 按进程放开（故意不按请求）。SECURITY.md 记下 DNS rebinding 等残余风险 |
| 4 | 每页抓两次 | `crawl()` 为了收链接把刚抓过的页面又 GET 一遍，等于每主机请求速率翻倍，而且第二次绕过了 `waitTurn` 限速——"礼貌爬虫"这条承诺被自己打破 | `fetchPage` 一次性返回结果与链接，`crawl` 复用；`InterfaceTest` 统计服务端收到的请求数把它钉住 |

另外补了 `SECURITY.md` 与 `CONTRIBUTING.md`：前者把"没鉴权是取舍不是漏洞"和取舍边界写清楚，后者把"仓库里每个数字都要能被复现"这条规矩写成可执行的检查清单。
